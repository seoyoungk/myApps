...
class LocationData: MainVC {
    ...
    // tmX, tmY를 이용하여 근접 측정소 목록 api 호출
    func callStationListAPI(tmX: Any, tmY: Any, completion: @escaping () -> Void) {
        let key = "Mj2lJctNluJLoMz0XV5F8XU0cGhTI2xNmVjB4fk%2BbojkGWq8%2F6PpOHbMVYrIKAxLQk8NgR7kPnJ%2BPD08HKqBEQ%3D%3D"
        let urlString = "http://openapi.airkorea.or.kr/openapi/services/rest/MsrstnInfoInqireSvc/getNearbyMsrstnList?tmX=\(tmX)&tmY=\(tmY)&ServiceKey=\(key)"
    ...
    override func parser(_ parser: XMLParser, foundCharacters string: String) {
        if stationblank == true && stationelementTemp != "response" && stationelementTemp != "header" && stationelementTemp == "resultCode", stationelementTemp != "00" {
            print("XML Error: Connect Success, Check resultCode!")
        }
